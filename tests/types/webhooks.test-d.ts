...
if (isKnownEvent(event)) {
  switch (event.type) {
    ...
    case "video.failed":
    case "video.deleted":
    case "video.source_scheduled_for_deletion":
    ...
    case "app.updated":
      ...
      break;
    // The spend-limit events share the "app." prefix but carry a notification
    // payload rather than an App snapshot, and the closed union types them as
    // such — a distinction a single `app.*` arm could not express.
    case "app.spend_limit_warning":
    case "app.spend_limit_exceeded":
      expectType<Equal<typeof event.data, SpendLimitNotification>>();
      break;
    default: {
